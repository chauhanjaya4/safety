...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
{
    ...
    $sql = "SELECT id,ed.username as uname,name,dept,desig,observ_date,observ_time,place,categories,type,potential,description,ed.status as ed_status,assign,remarks";
    ...
    $sql.=" where ed.username!=0 AND assign='".$_SESSION['username']."'";
    ...
    while ($row = $query->fetch_assoc()) {  // preparing an array
        $nestedData = array();  
        $id=$row['id'];
        $href='officers_action.php?id='.$id.'&username='.$row['uname'];
